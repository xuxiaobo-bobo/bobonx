...
def get_m(k_v):
    """读取js文件,获取加密值"""
    jscode = open('yrx_4.js', 'r', encoding='utf-8').read()
...
def td_list(res):
    ...
    for i in range(len(c)):
        td=res[c[i]:e[i]]
        tree=etree.HTML(td.replace('\\',''))
        src_list=tree.xpath('.//img/@class')
        for src in src_list:
            if img_num not in src:

                src_=tree.xpath(f'.//img[@class="{src}"]/@src')
                style_=tree.xpath(f'.//img[@class="{src}"]/@style')
                # print(style_)
                k_=[]
                for v_ in src_:
                    # print(v_)
                    for k,v in value.items():
                        if v_ == v:
                            k_.append(int(k))
                print('初始值',k_)  #['6', '0', '1', '8']
                dice_v={}

                offset_list = [int((float(style_[i].replace('left:','').replace('px','')))/11.5) for i in range(len(style_))]
                true_order_list = [None] * len(offset_list)
                # 排序
                for index, offset in enumerate(offset_list):
                    true_order_list[int(index + offset)] = str(k_[index])
                true_order_num = ''.join(true_order_list)
                print('end--------',true_order_num)
                sum_list.append(int(true_order_num))
                break
if __name__ == '__main__':
    ...
    for i in range(1,6):
        res = requests.get(f'https://match.yuanrenxue.com/api/match/4?page={i}',headers=headers)
        k_v = res.json()['key'] + res.json()['value']

        img_num=get_m(k_v)  #获取加密返回值


        # print(res.text)
        td_list(res.text)  #根据td分组
    print(sum(sum_list))
